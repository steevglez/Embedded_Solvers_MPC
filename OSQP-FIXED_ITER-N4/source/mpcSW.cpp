...
void mpcSW(elem x[N_SYS], elem yref[P_SYS], elem xmin[N_SYS], elem xmax[N_SYS], elem umin[M_SYS], elem umax[M_SYS], elem A[N_SYS][N_SYS], elem B[N_SYS], elem Acal[N_SYS*N_QP][N_SYS], elem AcalOmgOcal[N_SYS][N_QP], elem H[N_QP *N_QP], elem Mx[N_SYS*N_QP*2+2*N_QP *N_QP], elem L_last[N_SYS+P_SYS], int iterPDIP, int iterLS, elem tol, elem u[N_SYS], OSQPWorkspace * & work, OSQPSettings *settings, OSQPData *data,  int *started, double output_array[], tictoc *SOLVERTimer, int OUTPUT, int sample){
	...
	// ##### SOLVER AGREGADO: qpOASES #####
	osqpSW(H, h, Mx, cx, u_tilde, work, settings, data, started, SOLVERTimer);

	// MATLAB: u=u_tilde(1)+u_star;
	u[0]=u_tilde[0]+u_star[0];

	output_array[3*sample] = u_tilde[0];
	output_array[3*sample+1] = x[0]; // Revisar
	output_array[3*sample+2] = x[1]; // Revisar
