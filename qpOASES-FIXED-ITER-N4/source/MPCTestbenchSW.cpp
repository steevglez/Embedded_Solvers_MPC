...
int main(int argc, char *argv[]){
	...
	// Funcion de lazo MPC
        MPCTimer.tic();
        mpcSW(x_real, yref, xmin, xmax, umin, umax, A, B, Acal, AcalOmgOcal, H, Mx, L_invLast, u, &example, NUM_ITER, &started, output_array, &SOLVERTimer, sample);
        MPCTimer.toc();
    ...
    if(OUTPUT == 1){
        ...
        char format_csv_name_out[] = "csv_qpoasesMPC%sx%sOUT_iter%d.csv";
    ...
    if(TIME == 1){
    	...
    	char format_csv_name_time[] = "csv_qpoasesMPC%sx%sTIME_iter%d_%s.csv";
