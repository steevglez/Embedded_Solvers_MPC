...
int main(int argc, char *argv[]){
    ...
    // Cargar matriz A del sistema fisico
    for (int r=0; r<N_SYS; r++){
    ...
    // Cargar matriz de restricciones Mx - Cargada en orden distinto a los demas solvers
    ...
    fiordos4x4_Cparam_Params   params   = {0};
    fiordos4x4_Cparam_Settings settings = {0};
    fiordos4x4_Cparam_Result   result   = {0};
    fiordos4x4_Cparam_Work     work     = {0};
    fiordos4x4_Cparam_init(&params, &settings, &result, &work);
    
    // Configuracion del solver
    settings.approach.warmstartInner = 1;
    settings.algoInner.maxit = NUM_ITER;
    settings.algoOuter.maxit = NUM_ITER;
	...
	// Funcion de lazo MPC
        MPCTimer.tic();
        mpcSW(x_real, yref, xmin, xmax, umin, umax, A, B, Acal, AcalOmgOcal, H, Mx, L_invLast, u, &params, &settings, &result, &work, &started, output_array, &SOLVERTimer, sample);
        MPCTimer.toc();
	...
	// Nombre de archivo csv
        ...
        char format_csv_name_out[] = "csv_fiordosMPC%sx%sOUT_iter%d.csv";
	...
	// Nombre de archivo csv
    	...
    	char format_csv_name_time[] = "csv_fiordosMPC%sx%sTIME_iter%d_%s.csv";
