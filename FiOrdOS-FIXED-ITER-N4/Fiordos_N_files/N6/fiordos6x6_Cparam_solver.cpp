...
#include "fiordos6x6_Cparam_solver.h"
#include <stdio.h>
#include <math.h>
#include <string.h>
...
/* <<< copy vectors >>> */
static void copyvec6(realtype *dest, const realtype *src) {
    {
        int i;
        for (i=0; i<6; ++i) {
            dest[i] = src[i];
...
static void copyvec36(realtype *dest, const realtype *src) {
    {
        int i;
        for (i=0; i<36; ++i) {
            dest[i] = src[i];
...
/* <<< projection >>> */
static void algoOuter_project(realtype *thevec, fiordos6x6_Cparam_Work *work, fiordos6x6_Cparam_Params *params) {
    realtype *theveci;
...
/* <<< oracle >>> */
static void algoInner_oracle(fiordos6x6_Cparam_Work *work, fiordos6x6_Cparam_Params *params, realtype *z, realtype *fval, realtype *grad) {
    realtype hz[6];
    {
        /* (hz):= (params->H)^T*(z);   // (6x6)*(6x1) */
        int i,k;
        realtype tmp;
        for (i=0; i<6; ++i) {
            tmp = 0.0;
            for (k=0; k<6; ++k) {
              tmp += params->H[k+i*6]*z[k];
            }
            hz[i] = tmp;
        }
    }
...
static void algoOuter_oracle(fiordos6x6_Cparam_Work *work, fiordos6x6_Cparam_Params *params, realtype *v, realtype *fval, realtype *grad, fiordos6x6_Cparam_Settings *settings) {
    realtype *vineq;
    realtype *gradineq;
    vineq = v+0;
    gradineq = grad+0;
    ...
    {
        /* (work->Prob.inner_g):= (params->Ai)^T*(vineq)+(params->g);   // (6x36)*(36x1),(6x1) */
        int i,k;
        realtype tmp;
        for (i=0; i<6; ++i) {
            tmp = 0.0;
            for (k=0; k<36; ++k) {
              tmp += params->Ai[k+i*36]*vineq[k];
            }
            work->Prob.inner_g[i] = tmp + params->g[i];
        }
    }
    {
        /* (work->Prob.inner_c):= -(vineq)^T*(params->bi);   // (1x36)*(36x1) */
        int k;
        realtype tmp;
            tmp = 0.0;
            for (k=0; k<36; ++k) {
    ...
    /* AlgoFgmA */
    {
        work->algoInner.res_stopcode = 0;
        {
            ...
            copyvec6(z,work->algoInner.init);  /* (z):=(work->algoInner.init) */
            copyvec6(y,z);  /* (y):=(z) */
            alpha = 1;
            copyvec6(zp,z);  /* (zp):=(z) */
            ...
            copyvec6(work->algoInner.res_z,zp);  /* (work->algoInner.res_z):=(zp) */
    ...
    if (NULL!=grad) {
        {
            /* (gradineq):= (params->Ai)*(work->algoInner.res_z)-(params->bi);   // (36x6)*(6x1),(36x1) */
            int i,k;
            realtype tmp;
            for (i=0; i<36; ++i) {
                tmp = 0.0;
                for (k=0; k<6; ++k) {
                  tmp += params->Ai[i+k*36]*work->algoInner.res_z[k];
                }
...
void fiordos6x6_Cparam_solve(fiordos6x6_Cparam_Params *params, fiordos6x6_Cparam_Settings *settings, fiordos6x6_Cparam_Result *result, fiordos6x6_Cparam_Work *work) {
    ...
    /* << prepare data >> */
    {
        ...
        /* === compute global characteristics === */
        {
            ...
            work->algoInner.glob_LbL = 1.0/6*tr;
        ...
        /* === initial points === */
        copyvec6(work->algoInner.init,settings->algoInner.init);  /* (work->algoInner.init):=(settings->algoInner.init) */
        copyvec36(work->algoOuter.init,settings->algoOuter.init);  /* (work->algoOuter.init):=(settings->algoOuter.init) */
    ...
    /* AlgoFgmA */
    {
        work->algoOuter.res_stopcode = 0;
        {
            ...
            copyvec36(z,work->algoOuter.init);  /* (z):=(work->algoOuter.init) */
            copyvec36(y,z);  /* (y):=(z) */
            alpha = 1;
            copyvec36(zp,z);  /* (zp):=(z) */
            ...
            while (iter<settings->algoOuter.maxit) {
                ...
                while (1) {  /* restarting-loop */
                    ...
                    if ( 1==settings->approach.warmstartInner) {  /* reinit inner */
                        copyvec6(work->algoInner.init,work->algoInner.res_z);  /* (work->algoInner.init):=(work->algoInner.res_z) */
                    }
                    while (1) {  /* backtracking-loop */
                        for (icoord=0; icoord<36; ++icoord) {
                            zp[icoord] = y[icoord] + tstep*grady[icoord];
            ...
            copyvec36(work->algoOuter.res_z,zp);  /* (work->algoOuter.res_z):=(zp) */
    ...
    /* << finalize result >> */
    copyvec36(result->la,work->algoOuter.res_z);  /* (result->la):=(work->algoOuter.res_z) */
    algoOuter_oracle(work,params,work->algoOuter.res_z,&(result->d),NULL,settings);
    copyvec6(result->x,work->algoInner.res_z);  /* (result->x):=(work->algoInner.res_z) */
...
{int i; for (i=0;i<6;++i) {settings->algoInner.init[i] = (realtype) 0.0;}}
settings->algoInner.maxit = 100;
{int i; for (i=0;i<36;++i) {settings->algoOuter.init[i] = (realtype) 0.0;}}
